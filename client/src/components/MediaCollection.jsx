...
export default function MediaCollection({ items = [], onOpen, defaultView = "grid", className = "", cardActions }) {
  ...
  return (
    <section className={`media-collection ${className}`} aria-label="نتائج المكتبة" dir="rtl">
      ...
      <div className={view === "grid" ? "grid grid-cols-2 gap-3 min-[520px]:grid-cols-3 sm:grid-cols-[repeat(auto-fill,minmax(210px,1fr))] sm:gap-4 lg:gap-5" : "space-y-2.5 sm:space-y-3"}>
        {items.map((media) => (
