...
export default function MediaCollection({ items = [], onOpen, defaultView = "grid", className = "", cardActions }) {
  ...
  return (
    <section className={`media-collection ${className}`} aria-label="نتائج المكتبة" dir="rtl">
      ...
      <div className={view === "grid" ? "grid grid-cols-[repeat(auto-fill,minmax(250px,1fr))] gap-5 xl:grid-cols-[repeat(auto-fill,minmax(270px,1fr))]" : "space-y-2.5 sm:space-y-3"}>
        {items.map((media) => (
