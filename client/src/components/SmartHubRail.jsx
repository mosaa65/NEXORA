...
export default function SmartHubRail({ scope, title, description, onOpen }) {
  ...
  return <section className="space-y-4" dir="rtl"><div><h2 className="text-xl font-black text-[color:var(--text-primary)]">{title || "مجموعات ومحاور"}</h2>{description && <p className="mt-1 text-xs text-[color:var(--text-muted)]">{description}</p>}</div><div className="flex gap-3 overflow-x-auto pb-3 scrollbar-none snap-x touch-pan-x">{hubs.map((hub)=><div key={hub.slug} className="w-[min(84vw,340px)] shrink-0 snap-start sm:w-80"><SmartHubCard hub={hub} onOpen={onOpen} /></div>)}</div></section>;
}
