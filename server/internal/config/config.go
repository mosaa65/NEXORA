...
func Load() Config {
	...
	return Config{
		...
		ScanQueueSize:        envInt("NEXORA_SCAN_QUEUE_SIZE", 512),
		ScanMaxDepth:         envInt("NEXORA_SCAN_MAX_DEPTH", 0),
		ScanProgressInterval: time.Duration(envInt("NEXORA_SCAN_PROGRESS_SECONDS", 5)) * time.Second,
		FollowSymlinks:       envBool("NEXORA_SCAN_FOLLOW_SYMLINKS", false),
		IgnoreHidden:         envBool("NEXORA_SCAN_IGNORE_HIDDEN", true),
		IgnoreDirs:           envCSVList("NEXORA_SCAN_IGNORE_DIRS"),
		WatchRecursive:       envBool("NEXORA_WATCH_RECURSIVE", true),
		WatchDebounce:        time.Duration(envInt("NEXORA_WATCH_DEBOUNCE_MS", 2000)) * time.Millisecond,
		WatchStability:       time.Duration(envInt("NEXORA_WATCH_STABILITY_MS", 5000)) * time.Millisecond,
		WatchRetryInterval:   time.Duration(envInt("NEXORA_WATCH_RETRY_SECONDS", 15)) * time.Second,
		WatchErrorRetry:      time.Duration(envInt("NEXORA_WATCH_ERROR_RETRY_SECONDS", 5)) * time.Second,
		ReconcileInterval:    time.Duration(envInt("NEXORA_RECONCILE_INTERVAL_SECONDS", 900)) * time.Second,
...
// envCSVList reads a comma-separated list, used for configurable ignore rules.
