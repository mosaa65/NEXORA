...
func cacheRemoteImage(ctx context.Context, client *http.Client, imageURL, imageDir, provider, key string, mode ImageMode) (CacheImageResult, error) {
	...
	// Derive the final cache address before making an HTTP request. If the
	// file already exists, this makes repeated enrich/locale refreshes fully
	// local instead of re-downloading the same artwork from TMDB.
	...
	if stat, statErr := os.Stat(destination); statErr == nil && stat.Size() > 0 {
		return CacheImageResult{URL: "/assets/images/" + provider + "/" + fileName, LocalPath: destination, IsRemote: false}, nil
	}
