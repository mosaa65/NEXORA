...
- السلسلة لا تظهر للزائر إلا عند وجود **جزأين محليين حقيقيين على الأقل**. migration `0012_keep_provider_collection_counts_current.sql` تضيف trigger يحدّث `local_item_count` تلقائياً بعد إضافة أو حذف أي فيلم، بما في ذلك الحذف المتسلسل عند حذف `media_item`.
