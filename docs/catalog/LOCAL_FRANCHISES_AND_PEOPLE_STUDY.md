...
- تظهر السلسلة للزائر عند وجود **فيلم محلي واحد حقيقي على الأقل**؛ لذلك لا نخفي هوية الفيلم إذا كان أول جزء تضيفه إلى المكتبة. migration `0012_keep_provider_collection_counts_current.sql` تضيف trigger يحدّث `local_item_count` تلقائياً بعد إضافة أو حذف أي فيلم، بما في ذلك الحذف المتسلسل عند حذف `media_item`.
...
|---|---|---|---|
| سلسلة فيلم | `belongs_to_collection` في تفاصيل الفيلم | `collection.id` | رابط خفيف يعرّف السلسلة فوراً؛ لا نعتمد الاسم أبداً.
| تفاصيل السلسلة | `/collection/{collection_id}` بالعربية ثم الإنجليزية | `collection_id` | طلب منفصل كامل يوفر الاسم والوصف والأجزاء والصورة/الخلفية وتقييم كل جزء.
...
3. تفاصيل السلسلة لا تُجلب مكرراً: عند إثراء فيلم نربط `collection.id` أولاً، ثم نطلب `/collection/{id}` فقط إن لم توجد لقطة إنجليزية كاملة صالحة أو انتهت صلاحيتها (30 يوماً). التحديث اليدوي `POST /api/admin/franchises/:id/refresh` يجلب اللغتين قسراً، و`POST /api/admin/franchises/refresh-missing` يرقّي السلاسل القديمة على دفعات محدودة. لا توجد أي طلبات TMDB في صفحات العرض.
...
POST /api/admin/franchises/{id}/refresh
POST /api/admin/franchises/refresh-missing?limit=24
...
1. **واجهة لوحة التحكم المرئية**: API الإخفاء/الإبراز/الترتيب والتحديث موجودة، وما يبقى هو شاشة إدارية مرئية تجمعها مع المعاينة وتقارير الصور والعلاقات الناقصة.
2. **الشخصيات الخيالية المنظمة**: جدول مستقل وaliases وإدارة صريحة؛ لا مطابقة أسماء نصية غير موثوقة.
3. **اختبار واقعي موسع**: فيلم داخل سلسلة، مستقل، بلا ترجمة عربية، شخص ذو عدة أعمال، صورة محلية/غير موجودة، وإثراء متكرر لا يضاعف الروابط.
