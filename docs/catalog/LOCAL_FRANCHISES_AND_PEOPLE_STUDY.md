...
- بيانات `credits` يمكن جلبها ضمن لقطة العمل عند تفعيل وحدة الطاقم، لكنها ليست مفككة بعد إلى علاقات أشخاص محلية قابلة للبحث.
...
```text
GET  /api/franchises?scope=movies&limit=...
GET  /api/franchises/{slug}
GET  /api/franchises/{slug}/media
GET  /api/people?featured=true&limit=...
GET  /api/people/{slug}
GET  /api/people/{slug}/media

POST /api/media/{id}/enrich              # يزامن العلاقة تلقائياً
POST /api/admin/catalog/sync-relations   # طابور محدود لإعادة بناء العلاقات من اللقطات المحلية
PUT  /api/admin/franchises/{id}
PUT  /api/admin/people/{id}
PUT  /api/admin/media-credits/{id}
```
...
1. **الأساس الخلفي**: migration للجداول والفهارس، structs، repository، واختبارات upsert/link/deletion.
2. **مزامنة السلاسل**: تحليل `belongs_to_collection` من اللقطة الموجودة، ثم عميل TMDB لتفاصيل السلسلة واللغتين والصور المحلية.
3. **مزامنة الأشخاص والاعتمادات**: استخراج credits الموجودة، حدود `max_people_per_media`، وطابور تحديث مقتصد.
4. **API والبحث**: endpoints محلية، وربط حقول السلسلة/الأشخاص ذات الصلة في مستند البحث.
5. **واجهة المستخدم**: البطاقات والصفحات والصفوف باستخدام المكوّنات الموحدة فقط.
6. **لوحة التحكم والصيانة**: معاينة ما سيُحفظ قبل التحديث، إعادة بناء من snapshots بلا نت، تصحيح يدوي، تقارير الصور/العلاقات الناقصة.
7. **اختبار واقعي**: فيلم داخل سلسلة، فيلم مستقل، عمل بلا ترجمة عربية، ممثل له عدة أعمال محلية، صورة محلية/remote/غير موجودة، وإعادة إثراء لا تضاعف الروابط.
