...
| # | الفجوة | الأثر | الجدوى |
|---|---|
| 1 | `List()` يرجع خطأ "غير مدعوم" | لا تصفح للهاتف من الواجهة | قابلة للإصلاح |
| 2 | `Delete()` غير مدعوم | لا إدارة ملفات | قابلة (فيه `InvokeVerb('delete')` مستخدم فعلاً) |
| 3 | `Rename()` غير مدعوم | لا إعادة تسمية | **غير موثوق** عبر Shell COM |
| 4 | `PutStream` يمرّ بملف temp | كتابة مضاعفة، مساحة C: | **غير قابلة للإصلاح عبر Shell COM** |
| 5 | لا كشف فصل الجهاز | المهمة تعلّق حتى المهلة | قابلة (فحص دوري للجهاز) |
| 6 | لا فحص مساحة قبل البدء | فشل بعد بدء النسخ | قابلة |
| 7 | `Start-Sleep 500ms` بدل التحقق | فشل على أجهزة بطيئة | قابلة (polling بحد) |
| 8 | أول مساحة فقط | لا SD card | قابلة (تعداد المساحات) |
| 9 | الأخطاء نصية بلا أكواد | لا retry ذكي | قابلة (`TransferError`) |
