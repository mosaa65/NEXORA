...
| البند | الحالة | السبب |
|---|---|---|
| **مستندات الحلقة في البحث** | ❌ | جدول `episodes` وstream الـ cursor موجودان؛ استعلام الـ projection للحلقات يحتاج قرارًا في **شكل مستند الحلقة** (هل يحمل عنوان العمل؟ كيف يُرتّب؟) — قرار منتج لا قرار تقني |
